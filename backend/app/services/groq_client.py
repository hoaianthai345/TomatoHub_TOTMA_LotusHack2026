    ...
    def _post_chat_completions(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                f"Groq HTTPError {exc.code}: {detail or exc.reason}"
            ) from exc
        except URLError as exc:
            raise RuntimeError(f"Groq URLError: {exc.reason}") from exc
        ...
        if not isinstance(parsed, dict):
            raise RuntimeError("Groq response must be an object")
        return parsed
    ...
    def generate_json_object(
        ...
        *,
        ...
        try:
            ...
            # Compatibility fallback for models that may reject response_format.
